...
class FID_array(object):
	...
	def peakpicker(self,index=0):
        ...
        def convert_peaklist_to_ppm(self, sw_left=0):
                """ Convert a peaklist of array indices generated by peakpicker() to ppm"""
                if len(self.peaks[0])==0:
                    print "No peaks to convert."
                    return
                self.peaks = [self.conv_to_ppm(self.data[0], i, sw_left, sw_left-self.params['sw'])  for i in self.peaks]


        def convert_peaklist_to_index(self, ppm):
                """ Convert a peaklist of ppm generated by peakpicker() to array indices"""
                self.peaks = [self.conv_to_index(self.data[0], i, self.params['sw_left'], self.params['sw'])  for i in self.peaks]
	...
	def f_save(self,concs,rate,names,filename=None):
        ...
        @staticmethod
        def conv_to_ppm(data, index, ss_up, ss_dn):
            frc_sw = 1-index/float(len(data))
            ss_dn += frc_sw*(ss_up-ss_dn)
            return ss_dn

        @staticmethod
        def conv_to_index(data, ppm, sw_left, sw):
            frc_sw = (ppm+(sw-sw_left))/sw
            return int(ppm/sw*len(data))
